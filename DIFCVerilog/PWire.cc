...
: name_(n), type_(t), port_type_(pt), sectype_(st), maxtype_(mt), downtype_(nt), data_type_(dt),
  signed_(false), isint_(false), counter(1),
...
bool PWire::set_max_type(SecMaxType * mt)
{
	/* Legacy field; DIFCVerilog analysis does not consume it. */
	maxtype_ = mt;
	return true;
}

bool PWire::set_down_type(SecDownType* nt)
{
	/* Legacy field; DIFCVerilog analysis does not consume it. */
	downtype_ = nt;
	return true;
