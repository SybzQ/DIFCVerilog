...
class LexicalScope {
	...
	/* The scope has parameters that are evaluated when the scope
      ...
      struct param_expr_t : public LineInfo {
	    param_expr_t() : type(IVL_VT_NO_TYPE), msb(0), lsb(0), signed_flag(false), expr(0), range(0) , st(0) { }
	      // Type information
	    ...
	    range_t*range;
		SecType* st;
		 // Optional: add accessor methods
    void set_sec_type(SecType* sec_type) { st = sec_type; }
    SecType* get_sec_type() const { return st; }
      };
