...
int convert_verinum_to_int(const verinum& v) {
    unsigned long ulong_value = v.as_ulong();
    if (ulong_value <= static_cast<unsigned long>(INT_MAX)) {
        return static_cast<int>(ulong_value);
    } else {
        fprintf(stderr, "Error: verinum value is out of range for int type.\n");
...
static struct {
      ...
      SecType*sectype;
	  /* SecMaxType/SecDownType are accepted only for legacy syntax. */
	  SecMaxType*maxtype;
	  SecDownType*downtype;
      bool sign_flag;
      svector<PExpr*>* range;
} port_declaration_context = {NetNet::NONE, NetNet::NOT_A_PORT,
                              IVL_VT_NO_TYPE, new ConstType(), new SecMaxType (), new SecDownType(), false, 0};
...
%type <sectype> sec_label_opt
/* Legacy labels accepted by the parser but unused by DIFCVerilog. */
%type <maxtype> tre_label
%type <downtype> down_label
%type <set> integer_list1 integer_list2 integer_list3
...
sec_label
   : '{'integer_list1'|'integer_list2'|'integer_list3'}'
       {
             if ($4->empty()&& $6->empty()) {
                 // Both positive and negative capability sets are empty
                 // Use default SecType constructor for empty sets
                 //std::cout << "Empty set encountered." << std::endl;
                 $$ = new SecType(*$2,{},{}); // Use default constructor for empty sets
             }
			 else if($4->empty())
				 $$ = new SecType(*$2,{},*$6);
			 else if($6->empty())
               ...
               //  for (std::set<int>::iterator it = $1->begin(); it != $2->end(); ++it) {
                    // std::cout << *it << " ";
               //  }
                // std::cout << std::endl;
                 SecType* type = new SecType(*$2,*$4,*$6);
                 $$ = type;
             }
             //delete $2; // Don't forget to free the memory if needed
        }
    ...
    ;
integer_list1
    :  DEC_NUMBER ',' integer_list1
        {$$ = $3; // Recursive list construction
            $$ = $3;
			int myint= convert_verinum_to_int(*$1);
			//printf("shuzi:%d\n",myint);
            $$->insert($$->begin(), myint);
        }
    | DEC_NUMBER
...
tre_label
  : integer_list2
    {
      SecMaxType* type = new SecMaxType(*$1);
                 $$ = type;
    }
  | // use default label Low
    {
...
down_label
 : integer_list3
    {
      SecDownType* type = new SecDownType(*$1);
                 $$ = type;
    }
  | // use default label Low
    {
...
integer_list3
	...
	: attribute_list_opt K_reg
		...
		{ ivl_variable_type_t dtype = $3;
		  if (dtype == IVL_VT_NO_TYPE)
			dtype = IVL_VT_LOGIC;
		  pform_set_net_range_type($9, $5, $4, dtype, $6, $7 ,$8);
		  if ($1) delete $1;
		}
  ...
  /* This differs from the above pattern only in the absence of the
	...
	| attribute_list_opt K_reg
		...
		{ ivl_variable_type_t dtype = $3;
		  if (dtype == IVL_VT_NO_TYPE)
			dtype = IVL_VT_LOGIC;
		  pform_set_net_range_type($8, 0, $4, dtype, $5, $6, $7);
		  if ($1) delete $1;
		}
  ...
  /* Integer declarations are simpler in that they do not have all the
	 ...
	 //111
	| attribute_list_opt K_integer sec_label tre_label down_label register_variable_list ';'
		{ pform_set_reg_integer_type($6, $3, $4, $5);
		  if ($1) delete $1;
		}

	| attribute_list_opt K_time sec_label tre_label down_label register_variable_list ';'
		{ pform_set_reg_time_type($6, $3, $4, $5);
		}
	...
	| list_of_port_declarations ',' IDENTIFIER
		{ Module::port_t*ptmp;
		  ...
		  pform_module_define_port(@3, name,
					...
					port_declaration_context.maxtype,
					port_declaration_context.downtype,
...
port_declaration
  : attribute_list_opt
    K_input net_type_opt signed_opt range_opt sec_label tre_label down_label IDENTIFIER
      { Module::port_t*ptmp;
	perm_string name = lex_strings.make($9);
	...
	port_declaration_context.maxtype = $7;
	port_declaration_context.downtype = $8;
  ...
  | attribute_list_opt
    K_inout  net_type_opt signed_opt range_opt sec_label tre_label down_label IDENTIFIER
      { Module::port_t*ptmp;
	perm_string name = lex_strings.make($9);
	ptmp = pform_module_port_reference(name, @2.text,
					   @2.first_line);
  ...
  | attribute_list_opt
    K_output net_type_opt signed_opt range_opt sec_label tre_label down_label IDENTIFIER
      { Module::port_t*ptmp;
	perm_string name = lex_strings.make($9);
	ptmp = pform_module_port_reference(name, @2.text,
					   @2.first_line);
  ...
  | attribute_list_opt
    K_output var_type signed_opt range_opt sec_label tre_label down_label IDENTIFIER
      { Module::port_t*ptmp;
	perm_string name = lex_strings.make($9);
	ptmp = pform_module_port_reference(name, @2.text,
					   @2.first_line);
  ...
  | attribute_list_opt
    K_output var_type signed_opt range_opt sec_label tre_label down_label  IDENTIFIER '=' expression
      { Module::port_t*ptmp;
	perm_string name = lex_strings.make($9);
	ptmp = pform_module_port_reference(name, @2.text,
					   @2.first_line);
...
module  : attribute_list_opt module_start sec_label tre_label down_label IDENTIFIER
		{ pform_startmodule($6, @2.text, @2.first_line, $1,$3, $4, $5); }
	  ...
	  K_endmodule
		{ Module::UCDriveType ucd;
		  ...
		  pform_endmodule($6, in_celldefine, ucd);
		  delete[]$6;
		}
...
//
  /* This rule detects net declarations that possibly include a
	...
	: attribute_list_opt net_type
          ...
          delay3_opt
          sec_label tre_label down_label net_variable_list ';'

		{ ivl_variable_type_t dtype = $3;
		  if (dtype == IVL_VT_NO_TYPE)
			dtype = IVL_VT_LOGIC;
		  pform_makewire(@2, $5, $4, $10, $2,
				 NetNet::NOT_A_PORT, dtype, $7, $8,$9, $1);
		  if ($6 != 0) {
  ...
  /* Very similar to the rule above, but this takes a list of
	...
	| attribute_list_opt net_type
          primitive_type_opt signed_opt range_opt
          delay3_opt sec_label tre_label down_label net_decl_assigns ';'

		{ ivl_variable_type_t dtype = $3;
		  if (dtype == IVL_VT_NO_TYPE)
			dtype = IVL_VT_LOGIC;
		  pform_makewire(@2, $5, $4, $6,
				 str_strength, $10, $2, dtype, $7, $8, $9);
		  if ($1) {
  ...
  /* This form doesn't have the range, but does have strengths. This
	...
	| attribute_list_opt net_type
          primitive_type_opt signed_opt
          drive_strength sec_label tre_label down_label net_decl_assigns ';'

		{ ivl_variable_type_t dtype = $3;
		  if (dtype == IVL_VT_NO_TYPE)
			dtype = IVL_VT_LOGIC;
		  pform_makewire(@2, 0, $4, 0, $5, $9, $2, dtype, $6, $7, $8);
		  if ($1) {
			yyerror(@2, "sorry: Attributes not supported "
				"on net declaration assignments.");
			delete $1;
	...
	| K_trireg charge_strength_opt range_opt delay3_opt sec_label tre_label down_label list_of_identifiers ';'
		{ yyerror(@1, "sorry: trireg nets not supported.");
		  delete $3;
		  delete $4;
		}

	| port_type signed_opt range_opt delay3_opt sec_label tre_label down_label list_of_identifiers ';'
		{ pform_set_port_type(@1, $8, $3, $2, $1, $5, $6, $7);
		}
  ...
  /* The next two rules handle Verilog 2001 statements of the form:
	...
	| port_type net_type signed_opt range_opt sec_label tre_label down_label list_of_identifiers ';'
		{ pform_makewire(@1, $4, $3, $8, $2, $1, IVL_VT_NO_TYPE, $5, $6, $7, 0,
		                 SR_BOTH);
		}

	| K_output var_type signed_opt range_opt sec_label tre_label down_label list_of_port_identifiers ';'
		{ list<pair<perm_string,PExpr*> >::const_iterator pp;
		  list<perm_string>*tmp = new list<perm_string>;
		  for (pp = $8->begin(); pp != $8->end(); pp++) {
			tmp->push_back((*pp).first);
		  }
		  pform_makewire(@1, $4, $3, tmp, $2, NetNet::POUTPUT,
		                 IVL_VT_NO_TYPE, $5, $6, $7, 0, SR_BOTH);
		  for (pp = $8->begin(); pp != $8->end(); pp++) {
			if ((*pp).second) {
  ...
  /* var_type declaration (reg variables) cannot be input or output,
	...
	| K_input var_type signed_opt range_opt sec_label tre_label down_label list_of_identifiers ';'
		{ pform_makewire(@1, $4, $3, $8, $2, NetNet::PINPUT,
				 IVL_VT_NO_TYPE, $5, $6, $7, 0);
		  yyerror(@2, "error: reg variables cannot be inputs.");
		}

	| K_inout var_type signed_opt range_opt sec_label tre_label down_label list_of_identifiers ';'
		{ pform_makewire(@1, $4, $3, $8, $2, NetNet::PINOUT,
				 IVL_VT_NO_TYPE, $5, $6, $7, 0);
...
parameter_assign
  : sec_label IDENTIFIER '=' expression parameter_value_ranges_opt
      { PExpr*tmp = $4;
	pform_set_parameter(@2, lex_strings.make($2), param_active_type,
			    param_active_signed, param_active_range, tmp, $5,$1 );
	delete[]$2;
      }
  ;

localparam_assign
  : sec_label IDENTIFIER '=' expression
      { PExpr*tmp = $4;
	pform_set_localparam(@2, lex_strings.make($2), param_active_type,
			     param_active_signed, param_active_range, tmp,$1 );
	delete[]$2;
      }
  ;
sec_label_opt
  : sec_label { $$ = $1; }
  |           { $$ = new ConstType(); }  // Default security type
  ;
...
udp_port_decl
  ...
  | K_output sec_label tre_label down_label IDENTIFIER ';'
      { perm_string pname = lex_strings.make($5);
	PWire*pp = new PWire(pname, NetNet::IMPLICIT, NetNet::POUTPUT, $2, $3, $4, IVL_VT_LOGIC);
	svector<PWire*>*tmp = new svector<PWire*>(1);
	(*tmp)[0] = pp;
	$$ = tmp;
	delete[]$5;
      }
  | K_reg sec_label tre_label down_label IDENTIFIER ';'
      { perm_string pname = lex_strings.make($5);
	PWire*pp = new PWire(pname, NetNet::REG, NetNet::PIMPLICIT, $2, $3, $4, IVL_VT_LOGIC);
	svector<PWire*>*tmp = new svector<PWire*>(1);
	(*tmp)[0] = pp;
	$$ = tmp;
	delete[]$5;
      }
  | K_reg K_output sec_label tre_label down_label IDENTIFIER ';'
      { perm_string pname = lex_strings.make($6);
	PWire*pp = new PWire(pname, NetNet::REG, NetNet::POUTPUT, $3, $4, $5, IVL_VT_LOGIC);
	svector<PWire*>*tmp = new svector<PWire*>(1);
	(*tmp)[0] = pp;
	$$ = tmp;
	delete[]$6;
