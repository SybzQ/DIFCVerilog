...
void pform_startmodule(const char*name, const char*file, unsigned lineno,
		       svector<named_pexpr_t*>*attr,
                      SecType* sec, SecMaxType* tre, SecDownType* down)
{
	 ...
	 * a timescale directive. */
	  ...
	  pform_cur_module->set_labels(sec, tre, down);
		    ...
		    const char*file, unsigned lineno)
{
	...
	/* Make the PWire for the output port. */
      pins[0] = new PWire(out_name,
			  ...
			  NetNet::POUTPUT, new ConstType(), new SecMaxType (), new SecDownType(), IVL_VT_LOGIC);
      ...
      { list<perm_string>::iterator cur;
        ...
        for (cur = parms->begin(), idx = 1
	      ...
	      pins[idx] = new PWire(*cur, NetNet::WIRE,
				    NetNet::PINPUT, new ConstType(), new SecMaxType (), new SecDownType(), IVL_VT_LOGIC);
...
static void pform_set_net_range(perm_string name,
				...
				SecType* st,
				/* SecMaxType/SecDownType are legacy placeholders. */
				SecMaxType * mt,
				SecDownType * nt,
				PWSRType rt)
{
...
void pform_set_net_range(list<perm_string>*names,
			 ...
			 PWSRType rt)
{
	pform_set_net_range_type(names, range, signed_flag, dt, new ConstType(), new SecMaxType (), new SecDownType(), rt);
}
			 ...
			 PWSRType rt)
{
      ...
      for (list<perm_string>::iterator cur = names->begin()
		 ...
		 ; cur ++ ) {
	    perm_string txt = *cur;
	    pform_set_net_range(txt, range, signed_flag, dt, st, mt, nt, rt);
      }
			      ...
			      svector<named_pexpr_t*>*attr)
{
      ...
      cur = new PWire(name, type, port_type, sec_type, mt, nt, IVL_VT_LOGIC);
      FILE_NAME(cur, li);
	...
	SecDownType * nt )
{
	...
	// what the type is yet) then set the type now.
      ...
      if (! cur) {
	    new_wire_flag = true;
	    cur = new PWire(name, type, pt, st, mt, nt, dt);
...
void pform_makewire(const vlltype&li,
		    ...
		    PWSRType rt)
{
	pform_makewire(li, range, signed_flag, names, type, pt, dt, new ConstType(),new SecMaxType (), new SecDownType(), attr, rt);
}
		    ...
		    PWSRType rt)
{
      for (list<perm_string>::iterator cur = names->begin()
		 ...
		 ; cur ++ ) {
	    perm_string txt = *cur;
	    pform_makewire(li, txt, type, pt, dt, attr, st, mt, nt);
	...
	SecDownType * nt )
{
      ...
      while (first) {
	    ...
	    pform_makewire(li, first->name, type, NetNet::NOT_A_PORT, dt, 0, st, mt, nt);
	    /* This has already been done for real variables. */
	    if (dt != IVL_VT_REAL) {
		  pform_set_net_range(first->name, range, signed_flag, dt, st, mt,nt,
		                      SR_NET);
	    }
...
void pform_set_port_type(perm_string name, NetNet::PortType pt, SecType* st, SecMaxType * mt, SecDownType * nt,
			 const char*file, unsigned lineno)
{
      PWire*cur = pform_get_wire_in_scope(name);
      if (cur == 0) {
	    cur = new PWire(name, NetNet::IMPLICIT, NetNet::PIMPLICIT, st, mt, nt, IVL_VT_NO_TYPE);
				      ...
				      bool isint)
{
      ...
      for (list<perm_string>::iterator cur = names->begin()
	    ...
	    if (curw) {
		  ...
		  curw = new PWire(name, NetNet::IMPLICIT_REG, pt, new ConstType(), new SecMaxType (), new SecDownType(), vtype);
...
void pform_set_parameter(const struct vlltype&loc,
			 ...
			 LexicalScope::range_t*value_range,SecType* st)
{
      LexicalScope*scope = pform_get_cur_scope();
	...
	// Check if the parameter name is already in the dictionary.
      ...
      if (scope == pform_cur_module)
	   ...
	   if (st == nullptr)
        parm.st = new ConstType();  // ensure a valid security type is present
	  else parm.st=st;
}

void pform_set_localparam(const struct vlltype&loc,
			  perm_string name, ivl_variable_type_t type,
			  bool signed_flag, svector<PExpr*>*range, PExpr*expr,SecType* st)
{
      LexicalScope*scope = pform_get_cur_scope();
	...
	SecDownType *nt )
{
      for (list<perm_string>::iterator cur = names->begin()
		 ...
		 ; cur ++ ) {
	    perm_string txt = *cur;
	    pform_set_port_type(txt, pt, st, mt ,nt, li.text, li.first_line);
	    pform_set_net_range(txt, range, signed_flag, IVL_VT_NO_TYPE, st, mt, nt,
	                        SR_PORT);
      }
...
static void pform_set_reg_integer(perm_string name, SecType* st, SecMaxType * mt, SecDownType * nt)
{
      PWire*cur = pform_get_wire_in_scope(name);
      if (cur == 0) {
	    cur = new PWire(name, NetNet::INTEGER,
			    NetNet::NOT_A_PORT, st, mt,nt,
			    IVL_VT_LOGIC);
	    cur->set_signed(true);
	    pform_put_wire_in_scope(name, cur);
      } else {
...
{
	pform_set_reg_integer_type(names, new ConstType(), new SecMaxType (),new SecDownType ());
}

void pform_set_reg_integer_type(list<perm_string>*names, SecType* st, SecMaxType * mt,SecDownType * nt)
{
      for (list<perm_string>::iterator cur = names->begin()
		 ; cur != names->end()
		 ; cur ++ ) {
	    perm_string txt = *cur;
	    pform_set_reg_integer(txt, st, mt, nt);
      }
      delete names;
}

static void pform_set_reg_time(perm_string name, SecType* st, SecMaxType * mt,SecDownType *nt)
{
      PWire*cur = pform_get_wire_in_scope(name);
      if (cur == 0) {
	    cur = new PWire(name, NetNet::REG, NetNet::NOT_A_PORT, st, mt, nt, IVL_VT_LOGIC);
	    pform_put_wire_in_scope(name, cur);
      } else {
...
{
	pform_set_reg_time_type(names, new ConstType(), new SecMaxType (),new SecDownType ());
}

void pform_set_reg_time_type(list<perm_string>*names, SecType* st, SecMaxType * mt,SecDownType * nt)
{
      for (list<perm_string>::iterator cur = names->begin()
		 ; cur != names->end()
		 ; cur ++ ) {
	    perm_string txt = *cur;
	    pform_set_reg_time(txt, st, mt, nt);
      }
      delete names;
}
...
int pform_parse(const char*path, FILE*file)
{
	  ...
	  // Ensure stdin state is restored at the end of the function
