...
static void read_iconfig_file(const char*ipath)
{
      ...
      if (ifile == 0) {
	    ...
	    return;
      }// Open the file specified by ipath for reading. If fopen fails (returns NULL/0),
       // output an error message and return without further execution.

      while (fgets(buf, sizeof buf, ifile) != 0) {// read file content line by line into buf
	    ...
	    *cp++ = 0;
	    ...
	    } else if (strcmp(buf, "latticefile") == 0 || strcmp(buf, "depfunfile") == 0) {
		  continue;
...
extern void typecheck(map<perm_string,Module*> modules, list<perm_string> roots);
...
int main(int argc, char*argv[])
{
     signal(SIGSEGV, signal_handler);
      ...
      min_typ_max_flag = TYP;
      min_typ_max_warn = 10;
	  // getopt function for parsing command-line options
...
//for (const auto& elem : roots) {std::cout <<"roots:"<< elem << endl;;}
      if (roots.empty()) {
