...
class Module : public PScope, public LineInfo {
	...
	/* Initially false. This is set to true if the module has been
	   ...
	   other effect. */
      bool library_flag;
      perm_string instance_name_ = perm_string();
      perm_string parent_module_name_  = perm_string();
      bool is_cell;
      bool checked_ = false;
	...
	// The mod_name() is the name of the module type.
      ...
      void typecheck(ostream&out,ostream & PortsOut,ostream & AssignsOut,ostream & ALLnodesOut, TypeEnv& env,  map<perm_string,Module*>& modules, int current_index, std::vector<std::tuple<perm_string, perm_string, perm_string>>& child_modules) const;
      void CollectDepExprs(ostream&out, TypeEnv & env) const;
      bool is_checked() const { return checked_; }
      void mark_checked()  { checked_ = true; }

      void set_instance_name(const perm_string& name) { instance_name_ = name; }
       void set_parent_module_name(const perm_string& name) { parent_module_name_ = name; }

      const perm_string& get_instance_name() const { return instance_name_; }
      const perm_string& get_parent_module_name() const { return parent_module_name_; }

      // SecMaxType/SecDownType are legacy placeholders; DIFCVerilog uses SecType.
      void set_labels(SecType* s, SecMaxType* t, SecDownType* d) {
        sec_label_ = s;
        tre_label_ = t;
        down_label_ = d;
    }

    SecType* get_sec_label() const { return sec_label_; }
    ...
    private: // Not implemented
      ...
      SecType* sec_label_;
    // Legacy labels retained for parser/pform compatibility.
    SecMaxType* tre_label_;
    SecDownType* down_label_;
};
