...
{
    if(false) {
	cerr << "PExpr::get_name called on " << this << endl;

        // Check the 'this' pointer
        if (!this) {
            cerr << "ERROR: get_name called on null this" << endl;
            return perm_string();
        }

        // Check the virtual function table
        void** vtable = *(void***)this;
        if (!vtable) {
            cerr << "ERROR: null vtable in get_name" << endl;
            return perm_string();
        }
	}
    ...
    if (this == nullptr) {
        static perm_string empty_name = perm_string::literal("");
        return empty_name;
    }

    stringstream ss;
    dump(ss);
    std::string temp_str = ss.str();  // Use stack object to avoid memory leak
    ...
    return perm_string::literal(temp_str.c_str());
}
