...
ConstType* ConstType::TOP = new ConstType(10);
ConstType* ConstType::BOT = new ConstType(0);
...
{
	if (!comp1_->basic.empty() && !comp2_->basic.empty()) {
		comp1_->basic = unionSets(comp1_->basic, comp2_->basic);
		return comp1_;
	}

	if (comp1_->basic.empty()) {
		comp1_ = comp1_->simplify();
	}

	if (comp2_->basic.empty()) {
		comp2_ = comp2_->simplify();
	}
...
int JoinType::getSecvalue() {
	if (comp1_->secvalue >= comp2_->secvalue)
		return comp1_->secvalue;
	else if (comp1_->secvalue < comp2_->secvalue)
		return comp2_->secvalue;
	else return 0;
}
...
{
	if (!bexpr_) {
        cerr << "ERROR: Hypothesis::matches - bexpr_ is null!" << endl;
        return false;
    }
...
SecMaxType* SecMaxType::TOP = new SecMaxType();
SecMaxType* SecMaxType::BOT = new SecMaxType();
SecDownType* SecDownType::TOP = new SecDownType();
SecDownType* SecDownType::BOT = new SecDownType();
